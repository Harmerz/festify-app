    ...
    <div className="min-h-[100vh] w-full bg-background ">
      <div className="flex flex-col pb-20 pt-6">
        <div className="mb-2 flex flex-row items-center justify-between px-6">
          ...
          <h2 className="text-lg font-bold text-hitam">Top Rated</h2>
          <div className="flex flex-row items-center gap-2">
            <SortIcon />
          </div>
        </div>

        <div className="mt-6 flex flex-col gap-4 px-6 pb-10">
          {data.map((item, index) => (
            <div key={item.key} className="flex flex-row gap-3">
              <div className="absolute z-20 h-12 w-12">
                <TiStarburst
                  className={`absolute -left-4 -top-4 z-0 h-12 w-12 text-black ${
                    index === 0 ? 'text-primary' : ''
                  } ${index === 1 ? 'text-secondary' : ''} ${index > 1 ? 'text-white' : ''}`}
                />
                <p
                  className={`absolute -top-[5px] left-[2px] z-10 text-lg font-bold ${
                    index === 0 || index === 1 ? 'text-white' : 'text-black'
                  } `}
                >
                  {index + 1}
                </p>
              ...
              <div className="relative w-1/2">
                <div className="absolute bottom-2 left-2 z-50 flex flex-row items-center rounded-lg bg-white p-1 pr-2 text-xs font-medium text-hitam">
                  <svg
                ...
                <Image
                  src={`/assets/images/quick/result${item.image}.png`}
              ...
              <div className="flex w-1/2 flex-col gap-2 py-3">
                <div className="">
                  <p className="text-lg font-bold text-hitam">{item.title}</p>
                </div>
                ...
                <p className="line-clamp-2 text-xs text-hitam-700">{item.description}</p>
                <p className="text-sm font-bold text-hitam">IDR {item.price}</p>
              </div>
            </div>
          ))}
        </div>
